...
class OrderItem(models.Model):
    ...
    game = models.ForeignKey(Game, on_delete=models.CASCADE)
    game_key = models.OneToOneField(GameKey, on_delete=models.CASCADE, null=True, blank=True)
    price = models.DecimalField(max_digits=6, decimal_places=2)

    def __str__(self):
        return f"{self.game.title} (Key: {self.game_key.key_string if self.game_key else 'None'})"
